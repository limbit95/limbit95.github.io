...
import { drawingView } from "./views/drawing.js";
...
function render(state=store.get()){
 ...
 const s=state.snapshot;const isHost=s.me?.is_host===true;let stage;
 if(!s.round)stage=s.game?.status==="active"?roundWaitingView(s,isHost):setupView(s,isHost);
 else if(s.round.status===ROUND_STATUS.ROLE_REVEAL)stage=roleRevealView(s,state.myRole,isHost);
 else if(s.round.status===ROUND_STATUS.SPEAKING)stage=speakingView(s,isHost);
 else if(s.round.status===ROUND_STATUS.DRAWING)stage=drawingView(s,isHost);
 else if(s.round.status===ROUND_STATUS.DISCUSSION)stage=discussionView(s,isHost);
 else if([ROUND_STATUS.VOTING,ROUND_STATUS.RUNOFF_VOTING].includes(s.round.status))stage=voteView(s,state.voteState,state.myBallot,isHost);
 else if(s.round.status===ROUND_STATUS.VOTE_RESULT)stage=voteResultView(state.voteState,isHost);
 else if(s.round.status===ROUND_STATUS.LIAR_REVEAL)stage=captureRevealView(state.voteState,isHost);
 else if(s.round.status===ROUND_STATUS.LIAR_GUESS)stage=guessView(state.voteState,state.guessState);
 else if(s.round.status===ROUND_STATUS.ROUND_RESULT)stage=resultView(state.resultState,isHost);
 else stage="";
 const recallAllowed=canRecallRole(s);const cachedRole=state.myRoleRoundId===s.round?.id?state.myRole:null;const recallButton=recallAllowed?roleRecallButtonView(state.roleModalLoading):"";const recallModal=recallAllowed&&state.roleModalOpen&&cachedRole?roleRecallModalView(cachedRole,s.game?.game_mode):"";root.innerHTML=`${roomView(s,state.message,state.realtimeStatus)}<div class="game-stage" data-game-stage>${stage}${recallButton}</div>${recallModal}`;applyPostRenderMotion(root,state);
}
...
async function perform(task,{reload=true,recoverRoom=false}={}){store.set({message:""});try{await task();if(reload)await refresh();}catch(error){
 ...
 if(recoverRoom&&errorCode(error)==="ALREADY_IN_ACTIVE_ROOM"){try{await loadActiveRooms();}catch{}}
...
root.addEventListener("click",async(event)=>{
 ...
 if(action==="finish-drawing-turn")await perform(()=>commands.advanceDrawingTurn(s.round.version));
