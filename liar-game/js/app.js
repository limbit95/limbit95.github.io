...
function render(state=store.get()){
 ...
 const s=state.snapshot;const isHost=s.me?.is_host===true;let html=roomView(s,state.message,state.realtimeStatus);if(!s.round)html+=setupView(s,isHost);else if(s.round.status===ROUND_STATUS.ROLE_REVEAL)html+=roleRevealView(s,state.myRole,isHost);else if(s.round.status===ROUND_STATUS.SPEAKING)html+=speakingView(s,isHost);else if(s.round.status===ROUND_STATUS.DISCUSSION)html+=discussionView(s,isHost);else if([ROUND_STATUS.VOTING,ROUND_STATUS.RUNOFF_VOTING].includes(s.round.status))html+=voteView(s,state.voteState,state.myBallot,isHost);else if(s.round.status===ROUND_STATUS.VOTE_RESULT)html+=voteResultView(state.voteState,isHost);else if(s.round.status===ROUND_STATUS.LIAR_REVEAL)html+=captureRevealView(isHost);else if(s.round.status===ROUND_STATUS.LIAR_GUESS)html+=guessView(state.voteState,state.guessState);else if(s.round.status===ROUND_STATUS.ROUND_RESULT)html+=resultView(state.voteState,isHost);root.innerHTML=html;
}
...
async function perform(task,{reload=true,recoverRoom=false}={}){store.set({message:""});try{await task();if(reload)await refresh();}catch(error){
 ...
 if(["NOT_ROOM_MEMBER","ROOM_EXPIRED"].includes(errorCode(error))&&store.get().snapshot){clearVoteDraft();await unsubscribeRoomRealtime();setCurrentRoom("");store.set({snapshot:null,myRole:null,myRoleRoundId:null,voteState:null,guessState:null,myBallot:[],realtimeStatus:"closed"});try{await loadActiveRooms();}catch{}}
...
root.addEventListener("click",async(event)=>{const action=event.target.closest("[data-action]")?.dataset.action;if(!action)return;const s=store.get().snapshot;
 ...
 const leaveMessage=s?.me?.is_host?"방장이 나가면 이 게임방이 종료되고 모든 참가자가 방에서 나가게 됩니다.\n정말 방을 종료하시겠습니까?":"방에서 나가시겠습니까?";
  if(action==="leave"&&confirm(leaveMessage))await perform(async()=>{await commands.leaveRoom();clearVoteDraft();await unsubscribeRoomRealtime();setCurrentRoom("");store.set({snapshot:null,activeRooms:[],myRole:null,myRoleRoundId:null,voteState:null,guessState:null,myBallot:[],realtimeStatus:"closed"});},{reload:false});
