...
export function roleRecallModalView(role,gameMode=GAME_MODE.CLASSIC) {
  ...
  return `<div class="role-modal-backdrop" data-role-modal-backdrop role="presentation"><section class="role-modal" data-role-modal-panel role="dialog" aria-modal="true" aria-labelledby="role-modal-title"><header class="role-modal-header"><h2 id="role-modal-title">나의 역할</h2><button class="role-modal-close" data-action="close-role-modal" data-role-modal-close aria-label="역할 확인 창 닫기">×</button></header><p class="role-modal-role">${isLiar ? `🎭 ${hiddenRoleName}` : "시민"}</p>${category}${word}${team}<div class="role-modal-actions"><button data-action="close-role-modal">닫기</button></div></section></div>`;
}
