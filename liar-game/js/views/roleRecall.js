...
const RECALL_STATUSES = new Set([ROUND_STATUS.SPEAKING,ROUND_STATUS.DISCUSSION,ROUND_STATUS.VOTING,ROUND_STATUS.RUNOFF_VOTING,ROUND_STATUS.VOTE_RESULT,ROUND_STATUS.LIAR_REVEAL,ROUND_STATUS.LIAR_GUESS]);
...
export function roleRecallModalView(role) {
  ...
  return `<div class="role-modal-backdrop" data-role-modal-backdrop role="presentation"><section class="role-modal" data-role-modal-panel role="dialog" aria-modal="true" aria-labelledby="role-modal-title"><header class="role-modal-header"><h2 id="role-modal-title">나의 역할</h2><button class="role-modal-close" data-action="close-role-modal" data-role-modal-close aria-label="역할 확인 창 닫기">×</button></header><p class="role-modal-role">${isLiar ? "🎭 라이어" : "시민"}</p>${category}${word}<div class="role-modal-actions"><button data-action="close-role-modal">닫기</button></div></section></div>`;
}
