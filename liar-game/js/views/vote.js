...
export function voteBallotDetails(voteState){
 ...
 return `<div class="vote-detail"><h3>투표 상세</h3><div class="vote-detail-list">${ballots.map(ballot=>`<div class="vote-detail-item"><strong class="vote-detail-voter">${escapeHTML(ballot.voter)}</strong><span class="vote-detail-targets">→ ${(ballot.targets||[]).map(target=>escapeHTML(target.nickname)).join(", ")}</span></div>`).join("")}</div></div>`;
}
...
export function voteResultView(voteState,isHost){
 ...
 return `<section class="card stack"><h2>투표 결과</h2><ol class="vote-results">${tally.map(item=>`<li><strong>${escapeHTML(item.nickname)}</strong><span>${Number(item.votes)||0}표</span></li>`).join("")}</ol>${voteBallotDetails(voteState)}${runoff?`<div class="stack"><h3>동률이 발생했습니다.</h3><p>아래 후보를 대상으로 재투표가 필요합니다.</p>${locked.length?`<div><h3>확정된 의심 후보</h3>${peopleList(locked)}</div>`:""}<div><h3>재투표 후보</h3>${peopleList(boundary)}</div><p>남은 자리: <strong>${Number(voteState?.remaining_seats)||0}명</strong></p>${isHost?'<div class="row"><button class="secondary" data-action="start-runoff-speaking">한 바퀴 더 발언 후 재투표</button><button data-action="start-runoff">바로 재투표</button></div>':'<p class="notice">방장이 다음 진행 방식을 선택할 때까지 기다려 주세요.</p>'}</div>`:finalSuspects.length?`<div><h3>최종 의심자</h3>${peopleList(finalSuspects)}<p class="notice">라이어 검거 판정은 다음 단계에서 진행됩니다.</p></div>`:'<p class="notice">투표 결과를 확인할 수 없습니다.</p>'}</section>`;
}