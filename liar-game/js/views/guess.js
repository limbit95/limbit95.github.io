...
export function guessView(voteState,guessState){
 ...
 const history=guesses.length?`<div><h3>기존 추측</h3><ol class="guess-history">${guesses.map(guess=>`<li><strong>${guess.attempt_no}회차 · ${escapeHTML(guess.guesser)}</strong><blockquote>“${escapeHTML(guess.guess_text)}”</blockquote><span class="error">→ 오답</span></li>`).join("")}</ol></div>`:'<p class="muted">아직 제출된 추측이 없습니다.</p>';
