   ...
   <fieldset class="setup-fieldset" ${isHost?"":"disabled"}>
    ...
    <section class="setup-section setup-step setup-role-section">
     ...
     <div class="setup-role-options-grid">
      <label class="setup-control setup-role-count-control"><span>라이어 / 스파이 수</span><input name="liarCount" type="number" min="1" max="3" value="${liarCount}"><small>1~3명 · 시민은 최소 ${MIN_CITIZENS}명이 필요합니다.</small></label>
      <label class="setup-setting-row setup-role-toggle">
       ...
       <input name="showCategoryToLiar" type="checkbox" ${d.showCategoryToLiar===true?"checked":""}>
      </label>
      <label class="setup-setting-row setup-role-toggle">
    ...
    <section class="setup-section setup-step setup-progress-section">
     ...
     <div class="setup-rule-grid setup-flow-grid">
      ...
      <label class="setup-control" data-mode-only="drawing_spy">${controlTitle("그림 시간","한 사람당 5~60초로 설정합니다.")}<input name="drawingTimeLimit" type="number" min="5" max="60" value="${Number(d.drawingTimeLimit||15)}"></label>
      <label class="setup-control drawing-stroke-limit-control ${unlimitedStrokes?"is-unlimited":""}" data-mode-only="drawing_spy">${controlTitle("최대 획 수",unlimitedStrokes?"현재 획 수 무제한 모드입니다.":"한 사람당 1~10획으로 설정합니다.")}<input name="drawingStrokeLimit" type="number" min="1" max="10" value="${Number(d.drawingStrokeLimit||3)}" ${unlimitedStrokes?"readonly":""}></label>
      ...
      <label class="setup-control">${controlTitle("제시어 추측 횟수","붙잡힌 라이어/스파이 팀이 함께 공유하는 추측 기회입니다.")}<input name="guessLimit" type="number" min="1" max="3" value="${Number(d.guessLimit||1)}"></label>
     </div>
