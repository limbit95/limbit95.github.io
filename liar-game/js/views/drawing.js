...
export function drawingView(s,isHost){
 ...
 const remaining=Math.max(0,strokeLimit-used);
 const isCurrentDrawer=current?.player_id===s.me?.player_id;
 const canDraw=isCurrentDrawer&&remaining>0;
 ...
 return `<section class="card drawing-stage" data-drawing-stage>
  <header class="drawing-stage-header">
   ...
   <div class="drawing-turn-badges"><span>남은 시간 <strong data-drawing-timer>--</strong></span><span>남은 획 <strong data-drawing-strokes>${remaining} / ${strokeLimit}</strong></span></div>
  </header>
  ...
  <div class="drawing-board-shell ${canDraw?"is-active":"is-readonly"}">
   ...
   ${canDraw?'<div class="drawing-board-hint">검정 펜 · 한 번의 터치/드래그가 1획입니다</div>':'<div class="drawing-board-hint">현재 차례의 그림을 기다리고 있습니다</div>'}
  </div>
  ...
  <div class="drawing-layout-bottom">
   ...
   <aside class="drawing-rule-card"><h3>이번 라운드 규칙</h3><dl><div><dt>한 사람당 시간</dt><dd>${Number(drawing.time_limit||15)}초</dd></div><div><dt>최대 획</dt><dd>${strokeLimit}획</dd></div></dl><p>시간이 끝나거나 최대 획을 모두 사용하면 자동으로 다음 사람에게 넘어갑니다.</p></aside>
  </div>
