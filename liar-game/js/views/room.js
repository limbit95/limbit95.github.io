import { escapeHTML, GAME_MODE, MAX_ROOM_PLAYERS, MIN_CITIZENS, MIN_READY_PLAYERS } from "../constants.js";
...
export function roomView(s,message="",realtimeStatus="closed"){
 ...
 const liarCount=Math.max(1,Number(s.game?.liar_count||1));
 ...
 const canStart=!s.round&&s.game?.status==='setup'&&missingReady===0;
 const startControl=isHost&&!s.round&&s.game?.status==='setup'?`<div class="room-start-control"><button type="button" class="room-start-button" data-action="start-round" data-can-start="${canStart?"true":"false"}" ${canStart?"":"disabled"}>${canStart?"게임 시작":`게임 시작까지 ${missingReady}명이 더 필요합니다`}</button></div>`:"";
