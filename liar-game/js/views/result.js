...
function tallyRows(stage){
 const tally=[...(stage?.tally||[])].sort((a,b)=>Number(b.votes||0)-Number(a.votes||0)||String(a.nickname||"").localeCompare(String(b.nickname||""),"ko"));
 const maxVotes=Math.max(1,...tally.map(item=>Number(item.votes||0)));
 return tally.map((item,index)=>{
  const votes=Math.max(0,Number(item.votes||0));
  const width=Math.max(votes>0?12:0,Math.round(votes/maxVotes*100));
  return `<li class="result-vote-tally-row ${index===0&&votes>0?"is-top":""}">
   <div class="result-vote-tally-line"><span class="result-vote-rank">${index+1}</span><strong class="result-vote-name">${escapeHTML(item.nickname)}</strong><span class="result-vote-count">${votes}표</span></div>
   <div class="result-vote-meter" aria-hidden="true"><span style="--vote-width:${width}%"></span></div>
  </li>`;
 }).join("")||'<li class="result-vote-empty">득표 기록이 없습니다.</li>';
}

function voteHistory(stages=[],liarCount=1,hiddenRoleName="라이어"){
 return section("🗳️ 투표 과정",`<div class="result-vote-history">${stages.map(s=>`<article class="result-vote-stage"><header class="result-vote-stage-header"><h4>${stageTitle(s)}</h4><span>검거 대상 ${Number(liarCount)}명</span></header><div class="result-vote-block"><div class="result-vote-subheading"><h5>득표 순위</h5><small>막대가 길수록 많은 표를 받았습니다.</small></div><ul class="result-vote-tally">${tallyRows(s)}</ul></div><div class="result-vote-block"><div class="result-vote-subheading"><h5>누가 누구에게 투표했나</h5></div><div class="result-vote-ballots">${(s.ballot_details||[]).map(b=>`<div class="result-vote-ballot-row"><strong>${escapeHTML(b.voter)}</strong><span aria-hidden="true">→</span><span>${(b.targets||[]).map(t=>escapeHTML(t.nickname)).join(", ")||"선택 없음"}</span></div>`).join("")||'<p class="muted">상세 투표 기록이 없습니다.</p>'}</div></div>${s.runoff_required?`<div class="result-runoff-summary"><strong>재투표 발생</strong><p>확정된 지목: ${(s.stage_winners||[]).map(x=>escapeHTML(x.nickname)).join(", ")||"없음"}</p><p>동률 후보: ${(s.boundary_candidates||[]).map(x=>escapeHTML(x.nickname)).join(", ")}</p><p>남은 자리: ${Number(s.remaining_seats)}명</p></div>`:""}${(s.locked_winners||[]).length?`<p class="result-locked">이전 단계 확정: ${s.locked_winners.map(x=>escapeHTML(x.nickname)).join(", ")}</p>`:""}</article>`).join("")||'<p class="notice">투표 기록을 확인할 수 없습니다.</p>'}</div>`);
}
...
export function resultView(r,isHost,gameMode){
 ...
 let action="";
 if(citizen||captureSucceeded||revealed)action=isHost?'<button data-action="prepare-next-round">다음 라운드 준비</button><button class="secondary" data-action="restart-game">새 게임 · 설정 변경</button>':'<p class="muted result-next-waiting">방장이 다음 라운드 또는 새 게임을 준비할 때까지 기다려 주세요.</p>';
 const actionBlock=action?`<div class="result-next-actions">${action}</div>`:"";
 return `${actionBlock}<section class="card result-card ${citizen?"result-citizen":"result-liar"}" data-result-card data-result-id="${escapeHTML(r?.round_id||"")}" data-result-round="${Number(r?.round_no||0)}" data-result-winner="${citizen?"citizen":"liar"}" data-game-mode="${escapeHTML(resolvedMode)}" data-capture-succeeded="${captureSucceeded?"true":"false"}" data-liars-revealed="${revealed?"true":"false"}" data-finished-at="${escapeHTML(r?.finished_at||"")}" data-server-now="${escapeHTML(r?.server_now||"")}"><header class="result-hero"><h2 class="result-title" data-result-title>${citizen?"🏆 시민 승리!":`🎭 ${hiddenRoleName} 승리!`}</h2><strong>Round ${Number(r?.round_no||0)}</strong><p>${reasons[r?.result_reason]||"최종 결과가 확정되었습니다."}</p></header>${gameStatsSlot("result")}${section("제시어",`<p class="result-category">${escapeHTML(r?.category||"")}</p><p class="result-answer">${escapeHTML(r?.word||"")}</p>`)}${drawingSection}${comparison(r,revealed,hiddenRoleName)}${voteHistory(r?.vote_stages,Number(r?.liar_count||1),hiddenRoleName)}${guessSection}</section>`;
}
