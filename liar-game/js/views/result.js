...
export function resultView(r,isHost,gameMode=GAME_MODE.CLASSIC){
 const drawingMode=gameMode===GAME_MODE.DRAWING_SPY;const hiddenRoleName=drawingMode?"스파이":"라이어";
 ...
 return `<section class="card result-card ${citizen?"result-citizen":"result-liar"}" data-result-card data-result-id="${escapeHTML(r?.round_id||"")}" data-result-round="${Number(r?.round_no||0)}" data-result-winner="${citizen?"citizen":"liar"}" data-game-mode="${escapeHTML(gameMode)}" data-capture-succeeded="${captureSucceeded?"true":"false"}" data-liars-revealed="${revealed?"true":"false"}" data-finished-at="${escapeHTML(r?.finished_at||"")}" data-server-now="${escapeHTML(r?.server_now||"")}"><header class="result-hero"><h2 class="result-title" data-result-title>${citizen?"🏆 시민 승리!":`🎭 ${hiddenRoleName} 승리!`}</h2><strong>Round ${Number(r?.round_no||0)}</strong><p>${reasons[r?.result_reason]||"최종 결과가 확정되었습니다."}</p></header>${section("제시어",`<p class="result-category">${escapeHTML(r?.category||"")}</p><p class="result-answer">${escapeHTML(r?.word||"")}</p>`)}${comparison(r,revealed,hiddenRoleName)}${voteHistory(r?.vote_stages,Number(r?.liar_count||1),hiddenRoleName)}${guessSection}${action?`<div class="result-actions">${action}</div>`:""}</section>`;
}
