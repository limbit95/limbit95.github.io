...
export function resultView(r,isHost){
 ...
 let action;
 if(!citizen&&!captureSucceeded&&!revealed)action='<p class="muted result-auto-reveal-note">5초 카운트다운 후 실제 라이어가 자동으로 공개됩니다.</p>';
 else action=isHost?'<button data-action="prepare-next-round">다음 라운드</button><button class="secondary" data-action="restart-game">새 게임 · 설정 변경</button>':'<p class="muted">방장이 다음 라운드 또는 새 게임을 준비할 때까지 기다려 주세요.</p>';
