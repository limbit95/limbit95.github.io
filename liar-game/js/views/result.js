...
function comparison(r,revealed){
 return `<section class="result-comparison" data-result-section aria-label="최종 지목과 실제 라이어 비교">
  ...
  <article class="result-comparison-card is-liar"><span class="result-comparison-label">🎭 실제 라이어</span><div class="result-comparison-names">${revealed?names(r?.actual_liars||[],"확인 불가"):names([],"공개 전")}</div></article>
 </section>`;
}
...
export function resultView(r,isHost){
 ...
 let action;if(!citizen&&!revealed)action=isHost?'<button data-action="reveal-result-liars">라이어 공개</button>':'<p class="muted">방장이 라이어를 공개할 때까지 기다려 주세요.</p>';else action=isHost?'<button data-action="prepare-next-round">다음 라운드</button><button class="secondary" data-action="restart-game">새 게임 · 설정 변경</button>':'<p class="muted">방장이 다음 라운드 또는 새 게임을 준비할 때까지 기다려 주세요.</p>';
 return `<section class="card result-card ${citizen?"result-citizen":"result-liar"}" data-result-card data-result-id="${escapeHTML(r?.round_id||"")}" data-result-round="${Number(r?.round_no||0)}" data-result-winner="${citizen?"citizen":"liar"}"><header class="result-hero"><h2 class="result-title" data-result-title>${citizen?"🏆 시민 승리!":"🎭 라이어 승리!"}</h2><strong>Round ${Number(r?.round_no||0)}</strong><p>${reasons[r?.result_reason]||"최종 결과가 확정되었습니다."}</p></header>${section("제시어",`<p class="result-category">${escapeHTML(r?.category||"")}</p><p class="result-answer">${escapeHTML(r?.word||"")}</p>`)}${comparison(r,revealed)}${voteHistory(r?.vote_stages,Number(r?.liar_count||1))}${guessSection}<div class="result-actions">${action}</div></section>`;
}
