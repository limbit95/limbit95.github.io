...
export function resultView(voteState,isHost){
 ...
 const answer=voteState?.answer_category&&voteState?.answer_word?`<div><h3>제시어</h3><p class="muted">${escapeHTML(voteState.answer_category)}</p><p class="role-word">${escapeHTML(voteState.answer_word)}</p></div>`:"";
 ...
 return `<section class="card stack"><h2>${title}</h2><div><h3>최종 의심자</h3>${playerList(voteState?.final_suspects,"최종 의심자를 확인할 수 없습니다.")}</div><p>${summary}</p>${answer}${actualLiars}${next}</section>`;
}
