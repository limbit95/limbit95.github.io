...
export function hintShopView(role,{compact=false}={}){
  ...
  const items=offers.map(offer=>{
    ...
    return `<article class="hint-shop-item${purchased?" is-purchased":""}${known?" is-known":""}">
      ...
      ${purchased&&value?`<div class="hint-shop-value"><span>힌트</span><strong>${escapeHTML(value)}</strong></div>`:""}
  ...
  return `<section class="hint-shop${compact?" is-compact":""}" aria-label="힌트 상점">
    <header class="hint-shop-header"><div><span class="hint-shop-eyebrow">🎭 LIAR SHOP</span><h4>힌트 상점</h4></div><strong class="hint-shop-balance">🪙 ${balance}P</strong></header>
    ...
    <small class="hint-shop-footnote">힌트 코인은 이번 게임에서 라이어·스파이로 패배할 때 1P씩 획득합니다.</small>
  </section>`;
}
