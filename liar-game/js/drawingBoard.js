...
function strokePath(ctx,points){
 ...
 ctx.moveTo(Number(first.x)*WIDTH,Number(first.y)*HEIGHT);
 if(points.length===1){ctx.lineTo(Number(first.x)*WIDTH+.01,Number(first.y)*HEIGHT+.01);}
 else points.slice(1).forEach(point=>ctx.lineTo(Number(point.x)*WIDTH,Number(point.y)*HEIGHT));
 ctx.stroke();
}
...
async function submitStroke(points,canvas){
 ...
 try{
  ...
  if(code==="DRAWING_TIME_EXPIRED")void advanceWhenAllowed();
  else window.setTimeout(()=>drawPersisted(canvas),180);
 }finally{
...
async function advanceWhenAllowed(){
 ...
 try{await commands.advanceDrawingTurn(snapshot.round.version);}
  ...
  if(codeFor(error)!=="STALE_VERSION"&&String(error?.message||"").includes("요청을 처리 중")===false)setStatus(messageFor(error));
  autoAdvanceKey="";
...
function startTimer(){
 ...
 const update=()=>{
  ...
  if(remaining<=0){clearInterval(timer);timer=null;void advanceWhenAllowed();}
 };
 update();timer=setInterval(update,120);
}

function inspect(){
 const canvases=[...document.querySelectorAll("[data-drawing-canvas]")];
 canvases.forEach(bindDrawing);
