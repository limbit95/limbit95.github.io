...
test("strict online 2D bootstrap evaluates only the play-window entry", async () => {
  ...
  assert.deepEqual(imports, ["./playWindow.js?v=20260921-r15"]);
  assert.equal(documentObject.body.dataset.marbleBootstrapMode, "online-2d");
  assert.equal(documentObject.body.dataset.marbleBootstrapRevision, "20260921-r15");
});

test("online gameplay bootstrap restores dice charge, game exit, and ownership loader", async () => {
  const imports = [];
  const mode = await loadMarblePage({
    href: "https://example.test/marble-game/?play=classic&onlineRoom=room-1&marbleVisuals=full",
...
test("lobby route keeps the existing full module graph", async () => {
  const imports = [];
  const mode = await loadMarblePage({
