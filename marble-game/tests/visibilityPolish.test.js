...
const ownershipLoaderSource = readFileSync(new URL("../js/ownershipVisualLoader.js", import.meta.url), "utf8");
...
test("Classic ownership flag sits on the tile accent strip while edge trim stays outside", () => {
  assert.match(bootstrapSource, /ownershipVisualLoader\.js/);
  assert.match(ownershipLoaderSource, /themes\/classic\/ownershipVisual\.js/);
...
test("roll action exposes a centered hold-to-charge control", () => {
  ...
  assert.match(bootstrapSource, /diceCharge\.js/);
