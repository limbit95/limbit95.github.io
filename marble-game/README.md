...
현재 개발 위치는 **Phase 7A 경매 완료, Phase 7B 거래/협상 완료, Phase 7C 자산 매각 기반 파산 회피 Game Engine integration 진행 중**입니다.
...
현재는 **Phase 7C — Debt Recovery / Asset Liquidation**에 착수해 즉시 파산 전에 자산 매각으로 지불 가능성을 회복하는 독립 규칙 foundation을 추가하고 있습니다. basis point 기반 deterministic 매각가 계산 엔진과 Classic v1 규칙(도시 원가 50% + 누적 건설비 50% 환급)을 고정했고, 자산 선택 → READY → CONFIRMED lifecycle과 deterministic settlement를 구현했고, 현재 TOLL/TAX/EVENT 자금 부족을 opt-in debt recovery로 전환하는 Phase 7C Game Engine wrapper까지 연결하고 있습니다.
