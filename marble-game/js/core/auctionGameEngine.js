...
function resolveAuctionVote(state, action, options, {
  ...
  const announcementEndsAt = participantPlayerIds.length > 1
    ...
    : null;
  const selectorStopsAt = participantPlayerIds.length > 1
    ? deadlineAt(
      options,
      AUCTION_TIMING.startAnnouncementMs + AUCTION_TIMING.selectorSpinMs,
    )
    : null;
  const selectorResultEndsAt = participantPlayerIds.length > 1
    ? deadlineAt(
      options,
  ...
  if (participantPlayerIds.length === 1) {
    const autoPurchaseAuction = Object.freeze({
      ...timedAuction,
      bidPlayerIds: Object.freeze([starterPlayerId]),
      highestBid: vote.openingBid,
      highestBidderId: starterPlayerId,
      turnPlayerId: null,
      turnDeadlineAt: null,
      status: "WON",
      winnerPlayerId: starterPlayerId,
