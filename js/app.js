...
setBeforeRoute(async (routeInfo) => {
  ...
  if (requirement === "guest") {
    if (auth.user) {
      if (!auth.profile && ["/signup", "/login"].includes(routeInfo.path)) return true;
  ...
  if (requirement === "manager" && !auth.isAdmin && auth.managerCategoryIds.size === 0) {
    renderShellContent(routeInfo, el("div", { className: "page-container" }, accessDeniedState("활동 관리자 또는 카테고리 담당자만 이용할 수 있는 화면입니다.")));
    return false;
  }
  return true;
});
...
window.addEventListener("app:auth-changed", (event) => {
  ...
  if (["/signup", "/login"].includes(current) && auth.user && !auth.profile) return;
