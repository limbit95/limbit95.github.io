...
function openMapSearch(query) {
  window.open(mapSearchUrl(query), "_blank", "noopener,noreferrer");
}

function createMapShortcut(form) {
  const label = "네이버 지도에서 장소 검색";
  const button = document.createElement("button");
  button.type = "button";
  button.className = "activity-form__map-shortcut activity-form__map-shortcut--naver";
...
function enhanceActivityForm(form) {
  ...
  mapInput.placeholder = "https://naver.me/... 또는 https://map.naver.com/...";
  const row = document.createElement("div");
  row.className = "activity-form__map-link-row";
  mapInput.replaceWith(row);
  row.append(mapInput, createMapShortcut(form));
