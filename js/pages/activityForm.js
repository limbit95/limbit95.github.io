...
export async function renderActivityForm(route, mode) {
  ...
  const availableCategories = hasAdminPermission(auth, ADMIN_PERMISSION.OPERATIONS)
    ? categories
...
async function handleSubmit(submitEvent, context) {
  ...
  if (!hasAdminPermission(context.auth, ADMIN_PERMISSION.OPERATIONS) && !context.auth.managerCategoryIds.has(Number(form.category_id.value))) {
