...
test("No Thanks! Room/Lobby adapter maps RPC arguments without accepting client nickname authority", async () => {
  const client = fakeClient();
  const adapter = createNoThanksRoomLobbyAdapter({ client });
...
test("No Thanks! Room/Lobby adapter tracks authoritative room before Realtime invalidation", async () => {
  const client = fakeClient();
  const adapter = createNoThanksRoomLobbyAdapter({ client });
...
test("No Thanks! Room/Lobby adapter clears room subscription identity after leaving", async () => {
  const client = fakeClient();
  const adapter = createNoThanksRoomLobbyAdapter({ client });
...
test("No Thanks! Room/Lobby adapter rejects invalid player limits and versions before RPC calls", async () => {
  const client = fakeClient();
  const adapter = createNoThanksRoomLobbyAdapter({ client });
