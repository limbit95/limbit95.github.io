...
test("L folded ribbon gateway boots and Together enters the existing login flow", async ({ page }) => {
  const observed = observeBootFailures(page);

  await page.goto("/", { waitUntil: "domcontentloaded" });

  await expect(page).toHaveURL(/#\/gateway$/);
  await expect(page.getByRole("heading", { name: "청파 같이" })).toBeAttached();
  await expect(page.getByText("ONE WORD / THREE DIRECTIONS")).toBeVisible();
  await expect(page.getByRole("button", { name: "Like — 같이 닮다" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Value — 가치를 나누다" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Together — 같이 하다" })).toBeVisible();
  await expect(page).toHaveTitle("청파 같이 | Like · Value · Together");

  await page.getByRole("button", { name: "Together — 같이 하다" }).click();
