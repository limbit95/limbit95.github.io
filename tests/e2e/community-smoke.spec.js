...
test("M brand gateway boots first and Together enters the existing login flow", async ({ page }) => {
  const observed = observeBootFailures(page);

  await page.goto("/", { waitUntil: "domcontentloaded" });

  await expect(page).toHaveURL(/#\/gateway$/);
  await expect(page.getByRole("heading", { name: "하나의 같이, 세 개의 창." })).toBeVisible();
  await expect(page.getByText("DRAG WINDOWS / SHUFFLE THE VIEW")).toBeVisible();
