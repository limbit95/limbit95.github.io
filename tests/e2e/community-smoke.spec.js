...
test("brand gateway boots first and Together enters the existing login flow", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "더 약한 곁으로, 더 평화로운 쪽으로." })).toBeVisible();
  await expect(page.getByText("우리는 약한 이의 곁에 서고, 평화를 사랑하는 마음을 함께 배우고 실천합니다. 그 마음은 닮고, 나누고, 함께하는 세 가지 ‘같이’로 이어집니다.")).toBeVisible();
