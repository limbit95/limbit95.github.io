...
test("R aperture gateway loads and Together enters login", async ({ page }) => {
  const observed = observeBootFailures(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await expect(page).toHaveURL(/#\/gateway$/);
  await expect(page.getByRole("heading", { name: "같이", exact: true })).toBeVisible();
  await expect(page.getByText("A COMMUNITY SEEN THROUGH THREE APERTURES")).toBeVisible();
  await expect(page).toHaveTitle("청파 같이 | Like · Value · Together");
  await page.getByRole("link", { name: "ENTER TOGETHER" }).click();
