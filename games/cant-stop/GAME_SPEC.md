...
> UI/presentation은 같은 디렉터리의 `UI_DESIGN.md` adoption baseline과 이후 `UI_DECISIONS.md`의 최신 non-superseded 변경 결정을 함께 적용합니다.
> 실제 기능 구현 진행과 검증 상태는 같은 디렉터리의 `DEVELOPMENT.md`에서 관리합니다.
