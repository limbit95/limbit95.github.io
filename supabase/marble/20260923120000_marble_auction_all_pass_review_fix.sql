-- Keep competitive Marble auctions terminable when every participant passes before any bid.
-- Re-declare both action paths so repository migrations reproduce the current production bid behavior
-- and the deadline path closes the same no-bid/all-pass state as unsold.

CREATE OR REPLACE FUNCTION public.marble_auction_bid(p_room_id uuid, p_expected_version bigint, p_client_action_id uuid, p_amount integer DEFAULT NULL::integer, p_pass boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  v_replay := private.marble_action_replay(
    v_game.id,v_user,p_client_action_id,'auction_bid_v2',v_request
  );
  if v_replay is not null then return v_replay; end if;

  if v_game.status <> 'playing' or v_game.version <> p_expected_version then
    raise exception 'VERSION_CONFLICT';
  end if;
  if v_game.phase <> 'WAITING_CHOICE'
     or v_game.pending_choice->>'type' <> 'PROPERTY_AUCTION'
  then
  ...
  if (v_auction->>'turnDeadlineAt')::timestamptz <= now() then
    raise exception 'AUCTION_BID_DEADLINE_EXPIRED';
  end if;
  ...
  if v_player_id <> v_auction->>'turnPlayerId' then
    raise exception 'AUCTION_NOT_YOUR_BID_TURN';
  end if;
  ...
  v_minimum := case
    ...
    else (v_auction->>'openingBid')::integer
  end;
  ...
  if p_pass then
    ...
    if not (v_bids ? v_player_id) then
      v_bids := v_bids || jsonb_build_array(v_player_id);
    end if;
    ...
    v_events := jsonb_build_array(jsonb_build_object(
      ...
      'increase',v_increase,
      'surge',(v_previous_highest > 0 and v_increase >= v_surge_threshold)
    ));
  end if;
  ...
  if v_remaining = 0 and v_highest_bidder is null then
    ...
    where id=v_game.id;
        ...
        and v_highest_bidder is not null
        and not (v_passed ? v_highest_bidder)
  then
    if not p_pass then
    ...
    v_next := private.marble_auction_v2_next_turn(
      v_participants,v_passed,v_highest_bidder,v_player_id
    );
  ...
  perform private.marble_record_action(
    v_game.id,v_user,p_client_action_id,'auction_bid_v2',v_request,v_before,v_response
  );
  return v_response;
end;
$function$;
...
CREATE OR REPLACE FUNCTION public.marble_advance_auction_deadline(p_room_id uuid, p_expected_version bigint, p_client_action_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'private', 'pg_temp'
...
begin
  ...
  if v_type = 'AUCTION_VOTE' then
    ...
    if v_remaining=0 and v_auction->>'highestBidderId' is null then
      update public.marble_games
      set phase='TURN_END',
          pending_choice=null,
          last_events=v_events,
          ...
          updated_at=now()
      where id=v_game.id;
    elsif v_remaining=1 and v_auction->>'highestBidderId' is not null then
      perform private.marble_auction_v2_settle(v_game.id,v_auction,v_events,false);
    else
      v_next := private.marble_auction_v2_next_turn(
        v_participants,
