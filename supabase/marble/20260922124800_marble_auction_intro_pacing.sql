-- Refine Marble Auction intro pacing.
-- 2s announcement -> 3.2s roulette -> 2s result hold -> 2s first-bidder notice -> bidding.
...
begin
  ...
  if v_count > 1 then
    ...
    v_winner_notice_at := now() + interval '7.2 seconds';
    v_starts_at := now() + interval '9.2 seconds';
  end if;
