...
`20260829064240_the_game_stats_mvp`부터는 성공한 카드 제출을 서버에서 다시 검증한 뒤 `the_game_game_players`에 게임별 통계를 함께 누적합니다. 게임이 승리/패배로 확정되면 구원자, 카드 머신, 안정적인 손, 막판 해결사, 연쇄 플레이어, 위기관리 전문가, 과감한 승부사, 정밀 플레이어, 되돌리기 콤보의 9종 MVP를 서버에서 확정합니다. `the_game_get_game_stats`는 현재 종료 게임의 MVP와 플레이어별 세부 기록을, `the_game_get_my_stats`는 로그인 사용자의 누적 승률·연승·±10·플레이 성향·MVP 수상 횟수·최근 10게임을 반환합니다. 기존 게임 데이터는 세부 플레이 이벤트를 역산할 수 없으므로 통계 기능 도입 이후 종료된 게임부터 누적 기록에 포함합니다.
