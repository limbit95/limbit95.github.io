...
declare v_auth uuid:=auth.uid(); v_player public.liar_players%rowtype; v_room public.liar_rooms%rowtype; v_game public.liar_games%rowtype;
 v_count integer; v_round_no integer; v_round uuid; v_word public.liar_words%rowtype; v_last_word uuid; v_candidates integer;
begin
 if v_auth is null then raise exception using message='AUTH_REQUIRED',errcode='P0001'; end if;
 if p_player_key is null then raise exception using message='NOT_ROOM_MEMBER',errcode='P0001'; end if;
 ...
 -- TODO(PRODUCTION): 정식 배포 시 최소 준비 인원만 3명에서 4명으로 복구한다.
 -- 참가 인원별 라이어 수 강제 규칙은 없음.
 if v_count<3 then raise exception using message='NOT_ENOUGH_READY_PLAYERS',errcode='P0001'; end if;
...
as $$
declare v_auth uuid:=auth.uid(); v_player public.liar_players%rowtype; v_room public.liar_rooms%rowtype;
 v_round public.liar_rounds%rowtype; v_game public.liar_games%rowtype; v_new_game uuid; v_game_no integer;
begin
 if v_auth is null then raise exception using message='AUTH_REQUIRED',errcode='P0001'; end if;
 if p_player_key is null then raise exception using message='NOT_ROOM_MEMBER',errcode='P0001'; end if;
 -- Keep the lifecycle lock order consistent: player -> room -> round -> game.
 select lp.* into v_player from public.liar_players lp
 where lp.auth_user_id=v_auth and lp.player_key=p_player_key and lp.membership_status='active' for update;
 if not found then raise exception using message='NOT_ROOM_MEMBER',errcode='P0001'; end if;
 select rm.* into v_room from public.liar_rooms rm where rm.id=v_player.room_id for update;
 ...
 where lp.room_id=v_room.id and lp.membership_status='active';
 update public.liar_rooms rm set current_game_id=v_new_game,current_round_id=null,last_activity_at=now(),
  expires_at=now()+interval '24 hours',version=rm.version+1
 where rm.id=v_room.id returning rm.version into v_room.version;
 return query select v_new_game,v_game_no,v_room.version;
end $$;
...
as $$ declare v_auth uuid:=auth.uid(); v_player public.liar_players%rowtype; v_room public.liar_rooms%rowtype; v_result jsonb; v_is_spectator boolean;
begin
 if v_auth is null then raise exception using message='AUTH_REQUIRED',errcode='P0001'; end if;
 if p_player_key is null then raise exception using message='NOT_ROOM_MEMBER',errcode='P0001'; end if;
 select * into v_player from public.liar_players where auth_user_id=v_auth and player_key=p_player_key and membership_status='active';
 ...
 v_is_spectator:=v_room.current_round_id is not null and not exists(
  ...
  'me',jsonb_build_object('player_id',v_player.id,'nickname',v_player.nickname,'is_host',r.host_player_id=v_player.id,'is_spectator',v_is_spectator),
  ...
  'round',(select jsonb_build_object('id',x.id,'round_no',x.round_no,'status',x.status,'current_speaker_index',x.current_speaker_index,'current_vote_stage',x.current_vote_stage,'version',x.version,'spectator_category',case when v_is_spectator then x.category_snapshot else null end,'spectator_word',case when v_is_spectator then x.word_snapshot else null end) from public.liar_rounds x where x.id=r.current_round_id),
  'round_players',(select coalesce(jsonb_agg(jsonb_build_object('id',rp.id,'player_id',rp.player_id,'nickname_snapshot',rp.nickname_snapshot,'turn_order',rp.turn_order,'role_checked',rp.role_checked_at is not null,'is_liar',case when v_is_spectator then rp.role='liar' else null end) order by rp.turn_order),'[]'::jsonb) from public.liar_round_players rp where rp.round_id=r.current_round_id)
