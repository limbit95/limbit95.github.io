...
begin
  ...
  with correct_vote_totals as (
    select voter.player_id,
           (array_agg(voter.nickname_snapshot order by rd.round_no desc))[1] as nickname,
    ...
    where rd.game_id=v_game_id
      and rd.finished_at is not null
      and rd.winner in ('citizen','liar')
      and target.role='liar'
    group by voter.player_id
  ), peak as (
    select max(correct_votes) as max_count from correct_vote_totals
  )
  select case when coalesce(peak.max_count,0)>0 then jsonb_build_object(
      'count',peak.max_count,
      'players',coalesce((
  ...
  with liar_totals as (
    ...
    select max(role_count) as max_count from liar_totals
  )
  select case when coalesce(peak.max_count,0)>0 then jsonb_build_object(
      'count',peak.max_count,
      'players',coalesce((
