-- Phase 6-2: purchase a Development card from the current player's reserved cards.
...
alter table public.splendor_action_log
  ...
  check (action_type = any (array[
    ...
    'purchase_reserved'::text
  ]));

create or replace function public.splendor_purchase_reserved(
  p_room_id uuid,
...
begin
  ...
  if found then
    if v_existing.user_id = v_user_id and v_existing.action_type = 'purchase_reserved' then
  ...
  where gc.id = p_card_instance_id
    ...
    and gc.owner_game_player_id = v_player.id
  for update;

  if not found then
    raise exception 'RESERVED_CARD_NOT_AVAILABLE' using errcode = 'P0001';
  end if;
  ...
  v_was_hidden := v_card.reserved_hidden;
  ...
  set tokens = v_tokens,
      ...
      reserved_card_count = greatest(reserved_card_count - 1, 0)
...
revoke all on function public.splendor_purchase_reserved(uuid, uuid, jsonb, bigint, uuid) from public;
revoke all on function public.splendor_purchase_reserved(uuid, uuid, jsonb, bigint, uuid) from anon;
grant execute on function public.splendor_purchase_reserved(uuid, uuid, jsonb, bigint, uuid) to authenticated;
