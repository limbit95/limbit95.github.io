...
-- 기존 운영 notifications 스키마를 유지하면서 확장합니다.
-- 현재 운영 Supabase에 적용된 최종 구조와 동일한 참고용 패치입니다.
...
-- 쪽지 저장과 상대방 알림 생성을 한 트랜잭션으로 처리합니다.
...
-- 새 활동 등록 시 승인된 회원 모두에게 알림을 생성합니다.
-- 등록 후 2일이 지나면 프론트에서 '지난 알림'으로 분류됩니다.
...
revoke all on function private.notify_new_activity() from public, anon, authenticated;
...
-- 브라우저가 열릴 때의 보조 동기화 함수입니다.
-- 현재 로그인 사용자의 참여 확정 활동만 확인합니다.
...
-- 같은 이름으로 다시 schedule하면 기존 작업을 갱신합니다.
...
-- 새 notifications INSERT를 로그인 사용자의 브라우저가 즉시 받을 수 있도록 합니다.
do $$
begin
  if not exists (
    select 1
