...
현재 운영 DB에 최신 결과 공개 흐름과 제시어 풀을 반영하려면 아래 순서로 실행한다.
...
5. [ ] 자유 토론 채팅용 Realtime 송신 정책을 아직 적용하지 않았다면 `supabase/liar-game/realtime.sql` 실행
6. [ ] `게임`, `영화드라마`가 새 게임 설정 화면에서 보이지 않는지 확인
7. [ ] 각 남은 카테고리에 활성 제시어가 50개씩 존재하는지 확인
8. [ ] 검거 실패 시 5초 이전 자동 공개 RPC가 `RESULT_REVEAL_COUNTDOWN_ACTIVE`로 거부되는지 확인
...
6. [ ] `supabase/liar-game/functions-result.sql` — detailed result + timed failed-capture reveal RPC
7. [ ] `supabase/liar-game/migrations/20260824_refresh_word_pool.sql` — 최종 12카테고리 validation과 확장 word pool 적용
8. [ ] `supabase/liar-game/rls.sql` — base-table 차단과 authenticated RPC 권한
9. [ ] `supabase/liar-game/realtime.sql` — private state invalidation + ephemeral discussion chat authorization
10. [ ] 설치 후 schema/RPC 권한, 12카테고리 word pool, Production start matrix를 검증

기존 `20260821_*` migration들은 현재 `schema.sql`에 반영된 과거 변경이므로 fresh install에서 중복 적용하지 않는다. `20260824_refresh_word_pool.sql`만 현재 base seed/core를 최신 카테고리 정책으로 정리하기 위해 위 순서에서 적용한다.
