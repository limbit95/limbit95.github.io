...
revoke all on public.liar_hint_wallets from anon, authenticated;
revoke all on public.liar_hint_coin_events from anon, authenticated;
revoke all on public.liar_hint_purchases from anon, authenticated;
...
begin
  if new.status = 'ROUND_RESULT'
    ...
    with awarded as (
      ...
      select new.game_id,new.id,rp.player_id,1,'liar_loss_reward'
      ...
      returning game_id,player_id,delta
    )
    insert into public.liar_hint_wallets(game_id,player_id,balance,earned,spent,updated_at)
    select a.game_id,a.player_id,a.delta,a.delta,0,now()
    from awarded a
    on conflict (game_id,player_id) do update
      set balance = least(99, public.liar_hint_wallets.balance + excluded.balance),
          earned = public.liar_hint_wallets.earned + excluded.earned,
          updated_at = now();
  end if;
  return new;
end;
