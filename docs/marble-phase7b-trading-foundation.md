...
- 수신자는 제안을 수락하거나 거절할 수 있고, 제안자는 응답 전 자신의 제안을 취소할 수 있습니다.
...
최종 main 통합 검토에서 다음 두 안정성 문제를 보정했습니다.

- 거래 UI는 기존 온라인 컨트롤러/경매 UI와 동일한 canonical `onlineSession.js?v=20260910-r8` specifier를 사용해 import map이 가리키는 단일 session module instance를 공유합니다.
- 응답 없는 거래가 게임 진행을 영구 차단하지 않도록 제안자 전용 `TRADE_CANCEL` / `marble_trade_cancel` 경로를 추가했습니다. 자동 만료는 여전히 제품 규칙 미정 상태로 두며 임의 타이머를 추가하지 않습니다.
