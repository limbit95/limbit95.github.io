...
볼륨은 `localStorage`에 저장한다. Player의 slider 값은 사용자 설정값으로 그대로 유지하고, 게임별 `defaultOutputMultiplier`는 실제 Audio 출력 곡선에만 적용한다. 따라서 The Game의 기본 slider 값은 0.22를 유지하지만 실제 초기 출력은 0.44가 된다. 그 이후 구간은 100%까지 계속 증가하도록 보간해 slider 상단 구간도 유효하게 유지한다. 새 document가 생성되면 `hasEverPlayed`, interaction 대기 여부 같은 runtime 상태는 다시 초기화한다.
